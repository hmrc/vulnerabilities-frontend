...
Dependency context shows only affected versions reported as deployed by Releases API. Service assessments remain visible when no evidence versions match. If deployment information is unavailable, all supplied evidence is retained and labelled unavailable. The current contract does not identify an undeployed latest build.
